...
export const SpaceContainer: React.FC<{ index: number; spaceWidth: number; }> = ({ index, spaceWidth }) => {
    ...
    return (
        ...
        >
            ...
            <Text testID={'TileLabel'} style={{display: 'none', color: '#000'? '#fff': '#000'}}>{tile}</Text>
            {

               tile && <Image source={whitePawn} width={spaceWidth * 0.8} resizeMethod={'scale'} style={{width: spaceWidth * 0.7, height: spaceWidth * 0.7, alignSelf: 'center'}} />
            }
            <Tile />
        </View>
    );
