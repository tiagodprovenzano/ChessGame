...
export class TileAnimation {
    ...
    private static _toCallback:  ((config: ITileAnimationConfig) => void) | null= null
    ...
    static subscribe(callback: (value: any) => void, action: ITileAnimationActions = 'RENDER_ANIMATION'){
        switch (action) {
    ...
    private static triggerSubscriptions = (action: ITileAnimationActions) => {
        switch (action) {
            case 'RENDER_ANIMATION':
                this.subscriptions.RENDER_ANIMATION.forEach((c) => c(this._renderTileAnimation))
                break;
            case 'SET_ORIGIN_TILE':
    ...
    static setTileToMove = (position: IPositions, tileImageSrc: any ) => {
        ...
        if(currentPosition){
            this._tileToMove = {from: {x: currentPosition.x, y: currentPosition.y, id: position}, tileImageSrc}
    ...
    static moveTo = (position: IPositions) => {
        ...
        if(currentPosition && this._tileToMove){
            this._renderTileAnimation = {...this._tileToMove, to:{x: currentPosition.x, y: currentPosition.y, id: position}}
    ...
    static registerToCallback = (callback: (config: ITileAnimationConfig) => void) => {
